...
ALLOWED_CODE_EXTENSIONS = [".cpp"]
ALLOWED_NON_CODE_EXTENSIONS = [".png", ".jpg", ".jpeg", ".gif", ".pdf", ".docx"]
...
def filter_text_with_gemini(text: str) -> str:
    try:
        analysis_prompt = (
            "You are a specialized assistant designed to extract code snippets from mixed content. "
            "Given the following text which includes explanations, documentation, and code snippets, extract only the code snippets written in C++. "
            "The text may contain code in C++ only. Follow these instructions:\n\n"
            "1. **Identify Code Snippets**: Look for blocks of text that are likely code...\n\n"
            f"{text}\n\n"
            "Extract only the code snippets written in C++. Ensure the output is clean and well-formatted."
        )
