...
ALLOWED_CODE_EXTENSIONS = [".js"]  # Changed from .cpp to .js
...
def filter_text_with_gemini(text: str) -> str:
    try:
        analysis_prompt = (
            ...
            "Given the following text which includes explanations, documentation, and code snippets, extract only the code snippets written in JavaScript. "
            "The text may contain code in JavaScript only. Follow these instructions:\n\n"
            "1. **Identify Code Snippets**: Look for blocks of text that are likely code...\n\n"
            f"{text}\n\n"
            "Extract only the code snippets written in JavaScript. Ensure the output is clean and well-formatted."
        )
